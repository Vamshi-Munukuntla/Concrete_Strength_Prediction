...
def evaluate_regression_model(model_list: list,
                              ...
                              base_r2_score: float = 0.85) -> MetricInfoArtifact:
    try:
        index_number = 0
        ...
        for model in model_list:
            ...
            # calculating difference between train and test r2_scores
            diff_test_train_r2 = abs(test_r2 - train_r2)
            print(f"diff_test_train_r2: {diff_test_train_r2}")

            # logging all important metrics
            logging.info(f"{'>>' * 30} Scores {'<<' * 30}")
            logging.info(f"Train R2 Score \t\t Test R2 Score \t\t RMSE \t\t")
            logging.info(f"{train_r2}\t\t {test_r2}\t\t {rmse}")

            logging.info(f"Diff test train r2_score: [{diff_test_train_r2}")

            if test_r2 >= base_r2_score and diff_test_train_r2 < 0.10:
                base_r2_score = test_r2
        ...
        if metric_info_artifact is None:
            logging.info(f"No model found with higher r2 score than base r2 score")
...
class ModelFactory:
    ...
    # we're defining the class to call our model file (random forest and XGBoost)
    @staticmethod
    def class_for_name(module_name: str, class_name: str):
        try:
            # load the module, will raise Import error if module cannot be loaded
    ...
    def get_best_model_from_grid_searched_best_model_list(grid_searched_best_model_list: List[GridSearchedBestModel],
        ...
        try:
            ...
            for grid_searched_best_model in grid_searched_best_model_list:
                if base_r2_score < grid_searched_best_model.best_score:
                    logging.info(f"Acceptable model found: {grid_searched_best_model}")
                    base_r2_score = grid_searched_best_model.best_score
                    best_model = grid_searched_best_model

            if not best_model:
                raise Exception(f"None of the models has base r2 score: {base_r2_score}")
            logging.info(f"Best model: {best_model}")
            return best_model
        except Exception as e:
            raise CustomException(e, sys)

    def get_best_model(self, X, y, base_r2_score=0.85) -> BestModel:
        try:
